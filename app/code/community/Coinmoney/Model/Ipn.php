...
class Coinmoney_Model_Ipn extends Mage_Core_Model_Abstract
{
	function _construct()
	{
	...
	function GetStatusReceived($quoteId, $statuses)
	{
		...
		if (!$quote)
		{
			Mage::log('quote not found', NULL, 'coinmoney.log');
			return false;
		}
		
		$quoteHash = Mage::getModel('Coinmoney/paymentMethod')->getQuoteHash($quoteId);
		if (!$quoteHash)
		{
			Mage::log('Could not find quote hash for quote '.$quoteId, NULL, 'coinmoney.log');
			return false;		
		}
